...
//		nbJour = listCell.length,
		...
		gauche = marge + lgCol1,
		largJour = 6,
//		tabAt = [ "CS", "RC", "CS", "AT", "MP", "DIV", "F", "RP", "CP" ],
...
//			doc.rect(marge, h - delta, lgCol1, 6); 
			doc.text(marge+1, h, unEltTr.firstElementChild.firstElementChild.textContent);
			
			gauche = marge + lgCol1;
			
			doc.rect(gauche, h - delta, lgCol, 12); // filled square with red borders
			doc.text(gauche + 2, h, "conduite");
			doc.text(gauche + 2, 5 + h, "disque");
			
			gauche += lgCol;
			doc.rect(gauche, h - delta, lgCol, 12); // filled square with red borders
			doc.text(gauche + 2, h, "TA");
			doc.text(gauche + 2, 5 + h, "disque");
			...
			doc.rect(gauche, h - delta, lgCol, 12); // filled square with red borders
			doc.text(gauche + 2, h, "%");
			doc.text(gauche + 2, 5 + h, "Conduite");
			
			gauche += lgCol;
			doc.rect(gauche, h - delta, lgCol, 12); // filled square with red borders
			doc.text(gauche + 2, h, "Modif.");
			doc.text(gauche + 2, 5 + h, "disque");
...
//			doc.text( gauche + 2, h, ('     ' + listCell[1].firstElementChild.value.toFixed(2)).slice(-8) );
			doc.text( gauche + 2, h, ('     ' + new Number(listCell[1].firstElementChild.value).toFixed(2)).slice(-8) );
			
			gauche += lgCol;
			doc.setFillColor( arrayBgColor[0], arrayBgColor[1], arrayBgColor[2] );
			doc.rect(gauche, h - delta, lgCol, 6, 'FD');
			doc.text( gauche + 2, h, ('     ' + new Number(listCell[2].firstElementChild.value).toFixed(2)).slice(-8) );

			gauche += lgCol;
			doc.setFillColor( arrayBgColor[0], arrayBgColor[1], arrayBgColor[2] );
			doc.rect(gauche, h - delta, lgCol, 6, 'FD');
//			doc.text( gauche + 2, h, ('     ' + listCell[3].firstElementChild.value.toFixed(2)).slice(-8) );
			doc.text( gauche + 2, h, ('     ' + new Number(listCell[3].firstElementChild.value).toFixed(2)).slice(-8) );

			gauche += lgCol;
			doc.setFillColor( arrayBgColor[0], arrayBgColor[1], arrayBgColor[2] );
			doc.rect(gauche, h - delta, lgCol, 6, 'FD');
			doc.text( gauche + 2, h, ('     ' + new Number(listCell[4].firstElementChild.value).toFixed(2)).slice(-8) );

			gauche += lgCol;
			doc.setFillColor( arrayBgColor[0], arrayBgColor[1], arrayBgColor[2] );
			doc.rect(gauche, h - delta, lgCol, 6, 'FD');
			doc.text( gauche + 2, h, ('     ' + new Number(listCell[5].firstElementChild.value).toFixed(2)).slice(-8) );
			
			gauche += lgCol;
			doc.setFillColor( arrayBgColor[0], arrayBgColor[1], arrayBgColor[2] );
			doc.rect(gauche, h - delta, lgCol, 6, 'FD');
			doc.text( gauche + 2, h, ('     ' + listCell[6].firstElementChild.value).slice(-4) );
			...
			doc.rect(gauche, h - delta, lgCol, 6, 'FD');
			if(val.length)
				doc.text( gauche + 2, h, ( '     ' + new Number( val ).toFixed(2) ).slice(-8) );
			
			gauche += lgCol;
			doc.setFillColor( arrayBgColor[0], arrayBgColor[1], arrayBgColor[2] );
			doc.rect(gauche, h - delta, lgCol, 6, 'FD');
			doc.text( gauche + 2, h, ('     ' + listCell[8].firstElementChild.value).slice(-8) );
